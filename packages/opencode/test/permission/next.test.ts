...
it.live("permission requests from different directories are all visible globally", () =>
  Effect.gen(function* () {
    ...
    // Dispatch: globalPending is module-level, so list() returns ALL pending
    // entries regardless of which directory they were asked from.
    const allPending = yield* store.provide({ directory: one }, waitForPending(2))
    expect(allPending).toHaveLength(2)
    const ids = allPending.map((item) => item.id)
    expect(ids).toContain(PermissionV1.ID.make("per_dir_a"))
    expect(ids).toContain(PermissionV1.ID.make("per_dir_b"))

    // TUI can reply to any directory's permission from any directory context
    yield* store.provide({ directory: two }, reply({ requestID: PermissionV1.ID.make("per_dir_a"), reply: "reject" }))
    yield* store.provide({ directory: one }, reply({ requestID: PermissionV1.ID.make("per_dir_b"), reply: "reject" }))
