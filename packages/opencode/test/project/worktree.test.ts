    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          // Dispatch names worktrees as siblings: <repo-basename>-<slug>
          expect(info.name).toBe(`${path.basename(test.directory)}-my-feature`)
          expect(info.branch).toBe(info.name)
        }),
      { git: true },
    )
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.name).toBe(`${path.basename(test.directory)}-my-feature-branch`)
        }),
      { git: true },
    )
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          const test = yield* TestInstance
          const svc = yield* Worktree.Service
          yield* git(test.directory, ["branch", `${path.basename(test.directory)}-my-feature`])
    ...
    it.instance(
      ...
      () =>
        withCreatedWorktree({ name: "test-workspace" }, ({ info }) =>
          Effect.gen(function* () {
            const test = yield* TestInstance
            expect(info.name).toBe(`${path.basename(test.directory)}-test-workspace`)
            expect(info.branch).toBe(info.name)
          }),
        ),
      { git: true },
