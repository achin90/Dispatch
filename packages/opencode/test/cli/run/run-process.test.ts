  ...
  cliIt.concurrent(
    ...
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        ...
        // Dispatch's build agent asks for `bash` (upstream's allows it). `run` auto-rejects
        // permission asks, and a rejected tool call stops the loop before the next model
        // turn, so bash must be allowed for the failing stream to actually be reached.
  ...
  cliIt.concurrent(
    ...
    ({ llm, opencode }) =>
      Effect.gen(function* () {
        ...
        // See above: Dispatch's build agent asks for `bash`, and `run` auto-rejects asks,
        // which halts the loop. Allow bash so the second (failing) turn is reached.
