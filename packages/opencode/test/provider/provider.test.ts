...
import { Provider, SDK_MODELS } from "@/provider/provider"
...
// Dispatch: SDK model aliases are always injected into the anthropic provider,
// so provider filtering can never drop the anthropic provider entirely.
...
it.instance(
  "disabled_providers excludes upstream models but Dispatch SDK models remain",
  Effect.gen(function* () {
    yield* setProcessEnv("ANTHROPIC_API_KEY", "test-api-key")
    const anthropic = (yield* list)[ProviderV2.ID.anthropic]
    expect(anthropic).toBeDefined()
    const models = Object.keys(anthropic.models)
    for (const key of SDK_MODEL_KEYS) expect(models).toContain(key)
...
it.instance(
  "provider kept with only Dispatch SDK models when all upstream models filtered out",
  Effect.gen(function* () {
    const anthropic = (yield* list)[ProviderV2.ID.anthropic]
    expect(anthropic).toBeDefined()
    expect(Object.keys(anthropic.models).length).toBe(SDK_MODEL_KEYS.length)
  }),
...
it.instance(
  "enabled_providers with empty array leaves only the Dispatch SDK provider",
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    yield* set("OPENAI_API_KEY", "test-openai-key")
    const providers = yield* list
    expect(Object.keys(providers)).toEqual([ProviderV2.ID.anthropic])
    expect(Object.keys(providers[ProviderV2.ID.anthropic].models).length).toBe(SDK_MODEL_KEYS.length)
  }),
  { config: { enabled_providers: [] } },
)
