...
function withCreatedWorktree(
  ...
  const name = "api-test"
  // Dispatch creates worktrees as siblings named `<repo-basename>-<slug>`, with
  // branch === name (no `opencode/` branch prefix).
  const expected = `${path.basename(directory)}-${name}`
  const headers = { "content-type": "application/json" }
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      const ready = yield* waitReady({ name: expected }).pipe(Effect.forkScoped)
      ...
      expect(info).toMatchObject({ name: expected, branch: expected })
