        ...
        switch (value.type) {
          ...
          case "tool-call": {
            ...
            yield* status.set(ctx.sessionID, {
              type: "busy",
              activity: formatActivity(value.name, isRecord(value.input) ? value.input : undefined),
            })
          ...
          case "step-start":
            // Upstream removed the "start" stream event; "step-start" is the
            // first event of a model turn, so the busy/"Thinking..." activity
            // status moved here.
