...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cancel = Effect.fn("SessionPrompt.cancel")(function* (sessionID: SessionID) {
      ...
      const last = msgs.findLast((m) => m.info.role === "assistant")?.info as SessionV1.Assistant | undefined
      if (last) {
        const queued = msgs.filter((m) => m.info.role === "user" && m.info.id > last.id)
        if (queued.length === 1) {
          yield* Effect.logInfo("removing single queued message after cancel", { sessionID, messageID: queued[0].info.id })
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          if (model.providerID === "anthropic") {
            ...
            // (setStatus below is invoked from the stream processor, which is
            // not Effect code).
            ...
            const parts =
              lastUserMsg?.parts.filter((p): p is SessionV1.TextPart => p.type === "text" && !p.ignored) ?? []
            ...
            const media =
              lastUserMsg?.parts.filter((p): p is SessionV1.FilePart => p.type === "file" && supported.has(p.mime)) ??
              []
            ...
            const [skills, instructions] = yield* Effect.all([
              sys.skills(agent),
            ...
            yield* Effect.logInfo("claude-sdk: building plugin tool MCP server", {
            ...
            yield* Effect.logInfo("claude-sdk: resolved MCP servers", {
            ...
            if (agent.name === "plan") {
              const assistantParts = yield* MessageV2.parts(msg.id).pipe(
                Effect.provideService(Database.Service, database),
              )
              if (assistantParts.some((p) => p.type === "tool" && p.tool === "exitplanmode")) {
                yield* Effect.logInfo("runLoop: ExitPlanMode detected in plan agent turn, asking user to switch agent")
                ...
                if (answers && answers[0]?.[0] !== "Stay in Plan") {
                  ...
                  const switchModel = lastUser.model ?? (yield* provider.defaultModel().pipe(Effect.orDie))
                  const switchMsg: SessionV1.User = {
                    id: MessageID.ascending(),
                    sessionID,
                    role: "user",
                    time: { created: Date.now() },
                  ...
                  yield* Effect.logInfo("runLoop: switched agent after ExitPlanMode", { selectedAgent })
                }
              }
            }
...
export const node = LayerNode.make({
  ...
  deps: [
    ...
    RuntimeFlags.node,
    Database.node,
    // Dispatch: prompt.ts asks the user which agent to switch to after ExitPlanMode.
    Question.node,
  ],
})
