  ...
  Effect.gen(function* () {
    ...
    const cancel = Effect.fn("SessionPrompt.cancel")(function* (sessionID: SessionID) {
      ...
      if (last) {
        ...
        if (queued.length === 1) {
          yield* elog.info("removing single queued message after cancel", { sessionID, messageID: queued[0].info.id })
...
export const defaultLayer = Layer.suspend(() =>
  layer.pipe(
    ...
    // Split into a second .pipe(): Effect's pipe overloads cap at 20 arguments.
  ).pipe(
    Layer.provide(
      Layer.mergeAll(
        Agent.defaultLayer,
