 ...
 * Unlike the upstream `Instance.restore(instance, ...)` pattern, this looks up
 * the session's stored directory so that worktree agents get the correct context.
 */
export const withSessionInstance = <A>(sessionID: SessionID, fn: (instance: InstanceContext) => Promise<A>) =>
  Effect.gen(function* () {
    ...
    .post(
      ...
      async (c) =>
        jsonRequest("SessionRoutes.init", c, function* () {
          const sessionID = c.req.valid("param").sessionID
          const body = c.req.valid("json")
          yield* withSessionInstance(sessionID, () =>
    ...
    .post(
      ...
      async (c) =>
        jsonRequest("SessionRoutes.abort", c, function* () {
          const sessionID = c.req.valid("param").sessionID
          yield* withSessionInstance(sessionID, () =>
            AppRuntime.runPromise(SessionPrompt.Service.use((svc) => svc.cancel(sessionID))),
          )
          return true
        }),
    )
    ...
    .delete(
      ...
      async (c) =>
        jsonRequest("SessionRoutes.deleteMessage", c, function* () {
          const params = c.req.valid("param")
          yield* withSessionInstance(params.sessionID, () =>
            AppRuntime.runPromise(
              Effect.gen(function* () {
    ...
    .post(
      ...
      async (c) =>
        jsonRequest("SessionRoutes.command", c, function* () {
          ...
          return yield* withSessionInstance(sessionID, () =>
            AppRuntime.runPromise(
        ...
        const result =
          pid === "anthropic"
            ...
            : await AppRuntime.runPromise(
                withSessionInstance(sessionID, () => Summarize.aisdk(p, pid)),
              )
