          ...
          const reconnected = await bridge
            .promise(
              Effect.gen(function* () {
                ...
                if (stored.changed) yield* events.publish(ToolsChanged, { server: name }).pipe(Effect.catchCause(() => Effect.void))
    ...
    const authenticate = Effect.fn("MCP.authenticate")(function* (mcpName: string) {
      ...
      yield* Effect.tryPromise(() => open(result.authorizationUrl)).pipe(
        ...
        Effect.catch(() => {
          ...
          return events.publish(BrowserOpenFailed, { mcpName, url: result.authorizationUrl }).pipe(Effect.catchCause(() => Effect.void))
        }),
      )
