...
// MCP connections are created while the global layer is being built, where there is
// no AsyncLocalStorage Instance context, so InstanceState.directory would defect.
const currentDirectory = Effect.gen(function* () {
  return (yield* InstanceRef)?.directory ?? process.cwd()
})
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const baseDir = yield* currentDirectory
    ...
    function watch(s: State, name: string, client: MCPClient, bridge: EffectBridge.Shape, timeout?: number) {
      ...
      client.setNotificationHandler(ToolListChangedNotificationSchema, async () => {
        ...
        s.defs[name] = listed
        await bridge.promise(events.publish(ToolsChanged, { server: name }).pipe(Effect.catchCause(() => Effect.void)))
      })
    }
    ...
    function setupReconnect(s: State, name: string, client: MCPClient, bridge: EffectBridge.Shape) {
      ...
      client.onerror = (error) => {
        ...
        errorFired = true
        bridge.fork(
          Effect.logError("mcp client error, closing to trigger reconnect", {
            server: name,
            error: error.message,
      ...
      client.onclose = () => {
        ...
        bridge.fork(
          Effect.logWarning("mcp connection closed unexpectedly", {
            server: name,
            durationMs: Date.now() - connectedAt,
          }).pipe(Effect.ignore),
        )
        ...
        bridge.fork(events.publish(ToolsChanged, { server: name }).pipe(Effect.catchCause(() => Effect.void)))
          ...
          const reconnected = await bridge
            .promise(
              Effect.gen(function* () {
                ...
                const stored = yield* storeClient(
                  s,
                  name,
                  result.mcpClient,
                  result.defs,
    ...
    const prompts = Effect.fn("MCP.prompts")(function* () {
      return yield* collectFromConnected(s, McpCatalog.prompts, "prompts")
    })
    ...
    const finishAuth = Effect.fn("MCP.finishAuth")(function* (mcpName: string, authorizationCode: string) {
      ...
      const stored = yield* createAndStore(mcpName, { ...mcpConfig, enabled: true })
      return stored.status
    })
    ...
    const getAuthStatus = Effect.fn("MCP.getAuthStatus")(function* (mcpName: string) {
      const runtimeConfig = s.config[mcpName]
      ...
      yield* Effect.forEach(
        ...
        ([name, client]) =>
          Effect.gen(function* () {
            ...
            yield* Effect.logWarning("mcp health check failed, reconnecting", { server: name })
