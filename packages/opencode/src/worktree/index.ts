  ...
  Effect.gen(function* () {
    ...
    const candidate = Effect.fn("Worktree.candidate")(function* (input: {
      ...
      for (const attempt of Array.from({ length: MAX_NAME_ATTEMPTS }, (_, i) => i)) {
        ...
        // Dispatch: no `opencode/` branch prefix.
    ...
    const makeWorktreeInfo = Effect.fn("Worktree.makeWorktreeInfo")(function* (input?: {
      ...
      // Dispatch: worktrees are created as siblings of the current worktree,
      // not under Global.Path.data.
      ...
      return yield* candidate({ root: pathSvc.dirname(ctx.worktree), name: base, detached: input?.detached })
    })
