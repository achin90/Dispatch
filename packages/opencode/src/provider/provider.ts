  ...
  Effect.gen(function* () {
    ...
    const list = Effect.fn("Provider.list")(() =>
      InstanceState.use(state, (s) => {
        ...
        const id = ProviderV2.ID.make("anthropic")
        const sdkProv = sdkProvider()
        if (!providers[id]) {
          providers[id] = sdkProv
        } else {
          for (const [alias, model] of Object.entries(sdkProv.models)) {
    ...
    const getModel = Effect.fn("Provider.getModel")(function* (providerID: ProviderV2.ID, modelID: ModelV2.ID) {
      ...
      const sdkProv = sdkProvider()
      if (providerID === sdkProv.id) {
        const hit = sdkProv.models[modelID]
        if (hit) return hit
      }
      ...
      const anthropicId = ProviderV2.ID.make("anthropic")
      if (!providers[anthropicId]) {
        providers[anthropicId] = sdkProv
      } else {
        for (const [alias, model] of Object.entries(sdkProv.models)) {
