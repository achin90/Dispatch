...
// Dispatch's statically-defined SDK models. Upstream's version regex below does not
// match ids without a minor component ("claude-opus-5") or non-opus families.
...
// "claude-opus-4-7" — so match against a dash-normalised id.
...
function wantsSummarizedThinking(apiId: string): boolean {
  ...
  return anthropicOpus47OrLater(apiId) || SUMMARIZED_THINKING_MODELS.some((v) => id.includes(v))
}

function anthropicAdaptiveEfforts(apiId: string): string[] | null {
  if (wantsSummarizedThinking(apiId)) {
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  const id = model.id.toLowerCase()
  const adaptiveOpus = wantsSummarizedThinking(model.api.id)
