    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const methodImpl = Effect.fn("Installation.method")(function* () {
          if (process.execPath.includes(path.join(".dispatch", "bin"))) return "curl" as Method
          ...
          for (const check of checks) {
            ...
            const installedName =
              check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "opencode" : "dispatch-tui"
        ...
        const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
          ...
          if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
            ...
            const response = yield* httpOk.execute(
              HttpClientRequest.get(`${registry}/dispatch-tui/${channel}`).pipe(HttpClientRequest.acceptJson),
            )
        ...
        const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
          ...
          switch (m) {
            case "curl":
              ...
              break
            case "npm":
              result = yield* run(["npm", "install", "-g", `dispatch-tui@${target}`])
              break
            case "pnpm":
              result = yield* run(["pnpm", "install", "-g", `dispatch-tui@${target}`])
              break
            case "bun":
              result = yield* run(["bun", "install", "-g", `dispatch-tui@${target}`])
              break
            case "brew": {
              const formula = yield* getBrewFormula()
