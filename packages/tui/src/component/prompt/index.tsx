...
export function Prompt(props: PromptProps) {
  ...
  // When a draft is restored via ref.set(prompt, true), block submit until the
  // user presses a key. This prevents the Enter key that navigated into the
  // session from auto-submitting the restored draft.
  let restored = false
  ...
  async function submitInner() {
    ...
    if (store.mode === "shell") {
      ...
      // Resolve against the prompt's directory so worktree agent sessions see
      // their own directory-scoped commands, not just the TUI's.
      sync.command.for(promptDirectory()).some((x) => x.name === inputText.split("\n")[0].split(" ")[0].slice(1))
    ) {
      move.startSubmit()
    ...
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          <Switch>
            <Match when={status().type !== "idle" && !props.compact}>
              <box
                flexDirection="row"
                gap={1}
                flexGrow={1}
