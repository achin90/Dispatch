...
export function Autocomplete(props: {
  ...
  // Prefer an explicitly supplied directory over the ambient location: the
  // dashboard renders a Prompt for agent sessions that live in worktree
  // directories, outside of any session LocationProvider. Without this, file
  // suggestions resolve against the TUI's launch directory.
  ...
  function normalizeMentionPath(filePath: string) {
    const baseDir = resolvedLocation()?.directory || sync.path.directory || paths.cwd
  ...
  const [files] = createResource(
    () => ({ query: search(), location: resolvedLocation() }),
    async (input) => {
      if (!store.visible || store.visible === "/") return []
      if (referenceMatch()) return []
      const { lineRange, baseQuery } = extractLineRange(input.query ?? "")

      // Get files from SDK — the location resolves to the session's directory
      // so suggestions reflect the agent's working directory, not the TUI
      // launch directory.
