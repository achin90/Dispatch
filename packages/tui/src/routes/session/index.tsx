...
export function Session() {
  ...
  sdk.event.on("event", (evt) => {
    if (evt.payload.type !== "message.part.updated") return
  ...
  const bind = (r: PromptRef | undefined) => {
    ...
    if (resolution.action === "initial") {
      seeded = true
      r.set(route.prompt!, resolution.block)
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      run: async () => {
        ...
        const isQueued = pendingIndex !== undefined && messages().findIndex((item) => item.id === message.id) > pendingIndex
        if (!isQueued) {
          const status = sync.data.session_status?.[route.sessionID]
          if (status?.type !== "idle") await sdk.client.session.abort({ sessionID: route.sessionID }).catch(() => {})
        }
...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage }) {
  const ctx = use()
  // Dispatch: the Claude Agent SDK emits PascalCase tool names ("Read") and a couple of
  // aliases ("agent" for task, "code" for codesearch), so normalize before dispatching.
  const display = createMemo(() => {
    const tool = props.part.tool.toLowerCase()
    if (tool === "agent") return "task"
    if (tool === "code") return "codesearch"
    return toolDisplay(tool)
  })
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        {/* Dispatch: upstream dropped its codesearch tool, but the Claude SDK adapter still maps
            Claude's CodeSearch tool onto the "codesearch" tool id, so keep the renderer */}
        <Match when={display() === "codesearch"}>
...
// Upstream removed its codesearch tool, but the Claude SDK adapter still maps
// Claude's CodeSearch tool onto the "codesearch" tool id, so keep the renderer.
...
const toolDisplays = new Set([
  ...
  "skill",
  "execute",
  // Dispatch: Claude SDK code search
  "codesearch",
])
