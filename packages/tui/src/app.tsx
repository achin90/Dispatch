...
import { AgentSummaries } from "./component/agent-summaries"
...
export const run = Effect.fn("Tui.run")(function* (input: TuiInput) {
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
      ...
      // Dispatch: close any terminals attached to agent worktrees (dashboard).
      yield* Effect.addFinalizer(() => Effect.sync(PtyAttach.cleanup))
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const appCommands = createMemo(() =>
    [
      ...
      {
        name: "app.toggle.agent_summaries",
